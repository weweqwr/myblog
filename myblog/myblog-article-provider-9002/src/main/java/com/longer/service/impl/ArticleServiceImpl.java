...
public class ArticleServiceImpl implements ArticleService {
    ...
    public Map<String, Object> selectArticleByAccountOrTitle(String title, String author, int time, int browse, int state,int curPage,int showCount) {
        ...
        if(title==""||title==null) {
            if (state == 1) { //我的文章列表
                articleList = (List<Article>) redisRedis2Template.opsForHash().get(author + "articleList","articleList"+curPage+curPage);
            } else if (state == 0 && title == "" || title == null) {//草稿列表
                articleList = (List<Article>) redisRedis2Template.opsForHash().get(author + "articleListDraft", "articleListDraft"+curPage);
            }
            ...
            if (articleList == null) {
                ...
                //将数据存到redis中
                if (state == 1) { //我的文章列表
                    redisRedis1Template.opsForHash().put(author + "articleList","articleList"+curPage+curPage,articleList);
                    redisRedis1Template.expire(author + "articleList", 30, TimeUnit.DAYS);
                } else if (state == 0 && title == "" || title == null) {//草稿列表
                    redisRedis1Template.opsForHash().put(author + "articleListDraft", "articleListDraft"+curPage,articleList);
                    redisRedis1Template.expire(author + "articleList", 30, TimeUnit.DAYS);
                }
            }
        }else {
