...
//            存储rooleid 方便根据roleid查询判断是否根据那个sessionid登录
            ...
            Cookie[] cookies= request.getCookies();  Cookie cookie = new Cookie("sessionId", "");
            cookie.setPath("/");   //
...
//            存储rooleid 方便根据roleid查询判断是否根据那个sessionid登录
    ...
    public Map<String, Object> loginOut(HttpServletRequest request,HttpServletResponse response) {
        ...
        response.addCookie(cookie);
//            存储rooleid 方便根据roleid查询判断是否根据那个sessionid登录
        Cookie cookierole = new Cookie("roleid", "");
        cookierole.setPath("/");   //
        cookierole.setMaxAge(0);       //一小时
        response.setHeader("Access-Control-Allow-Credentials", "true");
        response.addCookie(cookierole);
    ...
    public Map<String, Object> selectHotUser() {
        ...
        userList=userDao.selectHotUser();
